...
| YuE2 score-first workflow (plan → review → render) | ✅ verified end to end | `--stage plan` → `plan/v1` in ≈19 s with no audio; `--from-plan` renders in ≈43 s; the plan-rendered take is **byte-identical** to the one-shot take (WAV/FLAC/MP3); an edited score changes the song and is carried through; `rendered_from` provenance in take metadata — [plans/2026-09-14-yue2-score-first-workflow.md](2026-09-14-yue2-score-first-workflow.md) |
| Absolute `--session` paths (WSL runtime mapping) | ✅ fixed | `Runtime.path()` returned None for drive-absolute paths, so absolute session paths failed outright; all three copies now map onto `/mnt/<drive>` |
